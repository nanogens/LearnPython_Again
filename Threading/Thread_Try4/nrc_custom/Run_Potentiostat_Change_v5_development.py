...
class Technique:
    def __init__(self,root_path,test_name,channel = 0):
    ...
    def ocv(self, rest_time_T, record_every_dE=0.1, record_every_dT=0.1, E_range='KBIO_ERANGE_AUTO', xctr=0):
    ...
    def ca(self, root_path, test_name, voltage_step, vs_initial, duration_step, channel = 0, n_cycles = 0, record_every_dI = 0.01, record_every_dt = 0.1,
        ...
        #Below populates the .exe command string and then runs the .exe
        cv_string = f'MFCStaticLink.exe -p {channel} CA {len(voltage_step) -1}' # -1 due to the offset of the first term being 0
    ...
    def cp(self,root_path, test_name, current_step, vs_initial, duration_step, channel = 0, n_cycles = 0, record_every_dE = 0.01, record_every_dt = 0.1,
        ...
        #Below populates the .exe command string and then runs the .exe
        cv_string = f'MFCStaticLink.exe -p {channel} CP {len(current_step) -1}' # -1 due to the offset of the first term being 0
