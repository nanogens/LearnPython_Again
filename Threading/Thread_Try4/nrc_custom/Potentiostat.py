...
# together with a Fluid Pump (Pump #6) at the same time.
# Refer to Threaded_Ops_Test.py (which is used on the North Robotics EMAP platform)
...
# change directory. Note this is the directory for when the MFCStaticLink.exe is located
...
class Technique:
    def __init__(self,root_path,test_name,directory,channel = 0):
        self.root_path = root_path
        self.test_name = test_name
        self.channel = channel
        self.directory = directory
        # directory is default 'C:/Users/Blackr/Documents/EC-Lab Development Package_Copy5/Examples/C-C++/VisualStudio/MFCStaticLink/output/' for USB0
        # directory is 'C:/Users/Blackr/Documents/EC-Lab Development Package_Copy5/Examples/C-C++/VisualStudio/MFCStaticLink/output_USB1/' for USB1 connection
        # If gives issue, can simply remove all directory info from this class and leave directory uncommented as above
        os.chdir(directory);

        # Check if the file already exists, to avoid overwriting:
        isFile = os.path.isfile(f'{root_path}{test_name}.json')
        if isFile == True:
            raise CustomError ('File name already exists, please rename to avoid ovewrite of existing data')
        else:
            holder = 'z'
    ...
    #reason you do not want the index to start at 0
    global expt_count
    global total_time
    expt_count = 0
    total_time = 0

    # def ocv(self, root_path, test_name, rest_time_T, channel = 0, record_every_dE = 0.1, record_every_dT = 0.1, E_range = 'KBIO_ERANGE_AUTO', xctr = 0):
    def ocv(self, root_path, test_name, rest_time_T, record_every_dE=0.1, record_every_dT=0.1, E_range='KBIO_ERANGE_AUTO', xctr=0):
        ...
        print(f'MFCStaticLink.exe -p {self.channel} OCV {rest_time_T} {record_every_dE} {record_every_dT} {E_range} {xctr}')
        os.system(f'MFCStaticLink.exe -p {self.channel} OCV {rest_time_T} {record_every_dE} {record_every_dT} {E_range} {xctr}')
        time_end = time.time() - time_start # Obtain total time length of the sequence

        with open(f'{self.directory}potentiostat_data.txt') as f:
            ...
            for row in data:
                ...
                i += 1
        with open(f'{self.root_path}{self.test_name}_OCV_{expt_count}_raw.json', 'w') as fp:
            json.dump(dict, fp, indent=4)
        expt_count += 1
        total_time = total_time + time_end

    def cv(self, root_path, test_name, vs_initial, voltage_step, scan_rate, record_every_dE, channel = 0, scan_number = 2, average_every_dE = 'FALSE', n_cycles = 0,
        ...
        cv_string = f'MFCStaticLink.exe -p {channel} CV '
    ...
    def ca(self, root_path, test_name, voltage_step, vs_initial, duration_step, channel = 0, n_cycles = 0, record_every_dI = 0.01, record_every_dt = 0.1,
        ...
        #Below populates the .exe command string and then runs the .exe
        ca_string = f'MFCStaticLink.exe -p {channel} CA {len(voltage_step) -1}' # -1 due to the offset of the first term being 0
        for i in range(len(voltage_step)):
            ca_string += f' {voltage_step[i]} {vs_initial[i]} {duration_step[i]}'
        ca_string += f' {n_cycles} {record_every_dI} {record_every_dt} {I_range} {E_range} {bandwidth} {xctr}'
    ...
    def cp(self,root_path, test_name, current_step, vs_initial, duration_step, channel = 0, n_cycles = 0, record_every_dE = 0.01, record_every_dt = 0.1,
           I_range = 'KBIO_IRANGE_1mA',E_range = 'KBIO_ERANGE_AUTO',bandwidth = 'KBIO_BW_5',xctr = 0):
        ...
        #Below populates the .exe command string and then runs the .exe
        cp_string = f'MFCStaticLink.exe -p {channel} CP {len(current_step) -1}' # -1 due to the offset of the first term being 0
        for i in range(len(current_step)):
            cp_string += f' {current_step[i]} {vs_initial[i]} {duration_step[i]}'
        cp_string += f' {n_cycles} {record_every_dE} {record_every_dt} {I_range} {E_range} {bandwidth} {xctr}'
    ...
    def geis(self, root_path, test_name, vs_initial,vs_final,initial_current_step,final_current_step, amplitude, frequency_number, final_frequency, initial_frequency,
             channel = 0, duration_step = 0, step_number = 0, record_every_dT = 0.1,record_every_dE = 0.01,sweep = 'False', average_n_times = 1, correction = 'False',
        ...
        os.system(f'MFCStaticLink.exe -p {channel} GEIS {vs_initial} {vs_final} {initial_current_step} {final_current_step} {duration_step} {step_number} '
             f'{record_every_dT} {record_every_dE} {final_frequency} {initial_frequency} {sweep} {amplitude} {frequency_number} '
             f'{average_n_times} {correction} {wait_for_steady} {I_range} {E_range} {bandwidth} {xctr}')
...
# TODO:
# - Add some useful information to the overwrite protection document